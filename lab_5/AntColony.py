...
class AntColony:
    ...
    def modify(self):
        ...
        for _ in range(number_to_modify):
            ...
            print("Edge " + str(i) + "-" + str(j) + " changed value from " + str(old_value) + " to " + str(new_value))
        if self.__best_cost is not None:
            self.__best_cost = 0
            for i in range(0, len(self.__best_path) - 1):
                u, v = self.__best_path[i], self.__best_path[i + 1]
                self.__best_cost += graph[u][v]["value"]

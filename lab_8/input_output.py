...
def read_sk():
    ...
    input_traits = data['feature_names']
    inputs = data['data'].tolist()
    index = 0
    for trait in input_traits:
        data_set.add_trait_and_values(trait, [my_input[index] for my_input in inputs])
        index += 1
    output_traits = data['target_names']
    outputs = data['target'].tolist()
    index = 0
    for trait in output_traits:
        data_set.add_trait_and_values(trait, [my_input[index] for my_input in outputs])
        index += 1
    my_traits = data_set.get_traits()
    for elem in my_traits:
        my_traits[elem] = utils.normalize_data(my_traits[elem])
    return data_set, input_traits, output_traits
...
def print_error_4_traits(data_set, trait1, trait2, trait3, trait4, my_regressors):
    """
    prints the error of a regression
    :param data_set: the training data set
    :param trait1: first trait
    :param trait2: second trait
    :param trait3: third trait
    :param trait4: fourth trait
    :param my_regressor: said regressor
    :return: None
    """
    predicted_outputs0 = [my_regressors[0].predict([x, y]) for x, y in
                          zip(data_set.get_trait_values(trait1), data_set.get_trait_values(trait2))]
    predicted_outputs1 = [my_regressors[1].predict([x, y]) for x, y in
                          zip(data_set.get_trait_values(trait1), data_set.get_trait_values(trait2))]
    correct_outputs = data_set.get_trait_values(trait3)
    error = performance_metrics.MSE(predicted_outputs0, correct_outputs)
    correct_outputs = data_set.get_trait_values(trait4)
    error += performance_metrics.MSE(predicted_outputs1, correct_outputs)
    print(error / 2)
    correct_outputs = data_set.get_trait_values(trait3)
    error = sklearn.metrics.mean_squared_error(correct_outputs, predicted_outputs0)
    correct_outputs = data_set.get_trait_values(trait4)
    error += sklearn.metrics.mean_squared_error(correct_outputs, predicted_outputs1)
    print(error / 2)
